  ...
  Query.datesClima = function (params) {
    return [

      {
        ...
        sql: "SELECT distinct data_previsao, data_previsao_utc, to_char(data_previsao, 'DD/MM HH24:MI') as f_data_previsao, to_char(data_atualizacao, 'DD/MM/YYYY') as f_data_atualizacao, to_char(data_modelo, 'DD/MM/YYYY') as f_data_modelo from v_clima_temperatura where data_atualizacao = (select max(data_atualizacao) from v_clima_temperatura) order by 1 ASC"
      },
      {
        id: 'next',
        sql: "select true"

...
class Update {
    getConfimadosSES(){
        ...
        request('http://datasets.saude.go.gov.br/coronavirus/casos_confirmados.csv')
            .then(function (csv) {
                fs.writeFile('confirmados.csv', csv.replace(/;/g , ','), function(err) {
                    if(err) {
    ...
    getObitosSES(){
        ...
        request('http://datasets.saude.go.gov.br/coronavirus/obitos_confirmados.csv')
            .then( async function (csv) {
                fs.writeFile('obitos.csv', csv.replace(/;/g , ','), async function(err) {
                    if(err) {
                        return console.log(err);
                    }else{
    ...
    async getSuspeitos(){
        ...
        let response = await request({url:url, qs:propertiesObject});
    ...
    async getDadosDF(){
        ...
        let response = await request('https://xx9p7hp1p7.execute-api.us-east-1.amazonaws.com/prod/PortalEstado');
    ...
    async up(callback) {
        ...
        setTimeout(async function () {
            ...
            let obitos = await self.getObitos();
            ...
            casos.confirmados.forEach(function(item, indice){
                ...
                let obtios     = self.findElement(obitos.confirmados, 'cd_geocmu', item.cd_geocmu);
                ...
                tabela.push({
                    ...
                    obitos: obtios == undefined ? 0 : obtios.obitos,
